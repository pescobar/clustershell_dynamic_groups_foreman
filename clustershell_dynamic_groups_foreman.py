...
class ClusterShellInventory(ForemanInventory):
    ...
    def parse_cli_args(self):
        ...
        parser = argparse.ArgumentParser(description='Generate clustershell dynamic groups based on foreman')
        parser.add_argument('--list-groups', action='store_true', default=False, help='List foreman host groups')
        parser.add_argument('--all-hosts', action='store_true', default=False, help='List all hosts in the foreman inventory')
        parser.add_argument('--group', action='store', help='List all hosts from group')
        parser.add_argument('--reverse', action='store', help='List all groups from host')
        parser.add_argument('--refresh-cache', action='store_true', default=False,
                            help='Force refresh of cache by making API requests to foreman (default: False - use cache files)')
        self.args = parser.parse_args()
